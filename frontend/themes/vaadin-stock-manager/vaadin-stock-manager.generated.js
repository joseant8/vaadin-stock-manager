...
window.Vaadin.Flow['_vaadintheme_vaadin-stock-manager_globalCss'] = window.Vaadin.Flow['_vaadintheme_vaadin-stock-manager_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_vaadin-stock-manager_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_vaadin-stock-manager_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_vaadin-stock-manager_globalCss'].push(target);
  }
  if (!document['_vaadintheme_vaadin-stock-manager_componentCss']) {
    
    document['_vaadintheme_vaadin-stock-manager_componentCss'] = true;
  }
